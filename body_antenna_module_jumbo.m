function [bodyMask,bodyCharacters, antennaMask,antennaCharacters]=body_antenna_module_jumbo(mask,wingParts,refPts,tipPts,bodyTrimPx, antTrimPx, scalelen)
%%
% bodyTrimPx=5;
%Calculate morphological parameters
disp('Begin to calculate morphology of body and antennae');
%Body
...
antenna=bwareafilt(imdilate(imerode(logical(bwareaopen(immultiply(antennaraw,upperSegMask),200)), strel('disk',bodyTrimPx)),strel('disk',bodyTrimPx)),2);
...
minimalAntennaLength=60; %The length of antennae should be longer than this length or it will be neglected
